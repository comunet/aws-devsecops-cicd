...
from AWSConfigRuleKMSStatementProcessor import AWSConfigRuleKMSStatementProcessor
    ...
    while response['Aliases']:
        for alias in response['Aliases']:
            if 'TargetKeyId' in alias:
                aliases.append(
                    {"alias_id": alias['AliasName'], "cmk_id": alias['TargetKeyId'], "account_number": re.search("([0-9]){12}", alias['AliasArn'])[0]}
                )
        if not 'NextMarker' in response:
            return aliases
        response = client.list_aliases(
            Marker=response['NextMarker'], Limit=100)
...
def cmk_alias_in_whitelist(alias_id, rule_parameters):
    try:
        ...
        whitelist_param = rule_parameters["CMKWhitelist"]
    except KeyError:
        return False
...
def evaluate_compliance(event, configuration_item, rule_parameters):
    AWS_KMS_CLIENT = get_client("kms", event=event)
    evaluations = []
    kms_aliases = cmk_aliases(AWS_KMS_CLIENT)
    if len(kms_aliases) > 0:
        for kms_alias in kms_aliases:
            ...
            elif is_cmk(AWS_KMS_CLIENT, kms_alias["cmk_id"]):
                # Evaluate if CMK key is disabled
                if cmk_status(AWS_KMS_CLIENT, kms_alias["cmk_id"]):
                    ...
                    processor = AWSConfigRuleKMSStatementProcessor(
                        get_cmk_policy(AWS_KMS_CLIENT, kms_alias["cmk_id"]))

                    result = processor.process(kms_alias["alias_id"], kms_alias["account_number"], rule_parameters)
...
def evaluate_parameters(rule_parameters):
    ...
    if "PrincipalWhitelist" not in valid_rule_parameters:
        valid_rule_parameters["PrincipalWhitelist"] = ""
