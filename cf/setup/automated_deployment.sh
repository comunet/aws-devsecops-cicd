...
then
    printf "\n[2 of 11] creating slack secrets in secrets manager\n"
    ...
    printf "\n[2 of 11] (STEP SKIPPED) no Group Chat integration selected\n"
fi

printf "\n[3 of 11] setup CodeCommit Repo in DEPLOYMENT account\n"
...
printf "\n[4 of 11] deploy the S3 Bucket for Build Artifacts with Policy + KMS Key to DEPLOYMENT Account\n"
...
printf "\n[5 of 11] get Copy of KMS Key Arn just created\n"
...
printf "\n[6 of 11] setup IAM Roles for CodePipeline to access DEPLOYMENT account\n"
...
printf "\n[7 of 11] deploy IAM Roles and KMS Trust with MANAGEMENT Account\n"
...
printf "\n[8 of 11] deploy StackSet Managed Self-service Roles to MANAGEMENT Account\n"
...
printf "\n[9 of 11] deploy Dynamo DB DevSecOps Configuration Tables to DEPLOYMENT Account\n"
...
printf "\n[10 of 11] compile Lambda Extension for Notifications Add-ons (Slack and MS Teams)"
# install node-lambda (if not already done)
npm install node-lambda -g
./cf/cicd/build_lambdas.sh


printf "\n[11 of 11] setup CI/CD Infrastructure Pipeline (CodePipeline) to DEPLOYMENT account\n"
