# AWS DevSecOps CI/CD Framework for AWS Organizations (v4)
...
- [AWS DevSecOps CI/CD Framework for AWS Organizations (v4)](#aws-devsecops-cicd-framework-for-aws-organizations-v4)
	- [1. About](#1-about)
	- [2. Prerequisite Setup:](#2-prerequisite-setup)
		- [2.6 Setup your AWS Delegated Admin Account by Registering from AWS Management Account (Refer to below provided link)](#26-setup-your-aws-delegated-admin-account-by-registering-from-aws-management-account-refer-to-below-provided-link)
			- [To register a delegated administrator (console)](#to-register-a-delegated-administrator-console)
	...
	- [4. Using the AWS DevSecOps CI/CD Framework](#4-using-the-aws-devsecops-cicd-framework)
		...
		- [4.2 Configuring DevSecOps Framework - Global Settings, Deployment Groups, Org Units and Stacks](#42-configuring-devsecops-framework---global-settings-deployment-groups-org-units-and-stacks)
			...
			- [4.2.2 OrgUnits.json](#422-orgunitsjson)
			- [4.2.3 Accounts.json](#423-accountsjson)
			- [4.2.4 DeploymentGroups.json](#424-deploymentgroupsjson)
			- [4.2.5 Stacks.json](#425-stacksjson)
...
The solution deploys DevSecOps code as CloudFormation StackSets in the 'DelegatedAdminAccount' which then deploy Stack Instances to specified TEST and PROD AWS Organisation OrgUnitIds in specified regions.
...
- [org-deployment] (+ Delegated Admin Account) - the AWS Account to host the Code Repo (CodeCommit), CICD components (CodePipeline, CodeBuild) and Create and manage Stacksets. 

### 2.5 Setup your AWS Organisations Management for enabling StackSets <!-- omit in toc -->
...
### 2.6 Setup your AWS Delegated Admin Account by Registering from AWS Management Account (Refer to below provided link)
As from: [https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/stacksets-orgs-delegated-admin.html?icmpid=docs_cfn_console]
...
4. In the Register delegated administrator dialog box, choose Register delegated administrator. Set the 'Delegated administrator account ID' to the Account Id of your Deployment Account (org-deployment).
5. The success message indicates that the member account has successfully been registered as a delegated administrator.
...
2. Deploy TEST StackSets to Deployment Account (Delegated Admin Account)
...
5. [if approval recieved] 
   1. Deploy PROD StackSets to Deployment Account (Delegated Admin Account)
...
> NOTE: The sample stack the solution comes with demonstrates setting up a simple S3 Bucket to target TEST and PROD OU's centrally from the AWS Deployment Account, as compiled and deployed from the DEPLOYMENT account. Replace this sample with your own stacks and resources.
...
As well as being used by the CDK, by storing the json files in DynamoDB this allows you to build other solutions (such as Lambda Scripts) that can later access the current DevSecOps deployment settings if necessary.
...
These parameters do not need to be referenced within CloudFormation Templates (`Resources` section), but they must exist in the `Parameters` sections of the linked templateFile.
...
#### 4.2.3 Accounts.json
If you are targeting StackSets by individual AWS Accounts (instead of OrgUnits - the default approach), then this file is used to specify which AWS Regions you want to deploy the stack to within that Account.

Whenever you specify a new accountIds in [DeploymentGroups.json] 'targetRegions', this file needs to be updated to include this new `accountId` and the regions you want DevSecOps code deployed to this account.

Within a given AccountId, list all regions you will deploying resources in a list of `deploymentRegions`.

Some StackSets may contain resources that can only be installed once per Account. Specify the `defaultRegion` where you want such global resources deployed. In the `Stacks.json` for the given StackSet specify `overrideDeploymentRegionUseDefaultOnly` to deploy to this `defaultRegion` only.

#### 4.2.4 DeploymentGroups.json
[DeploymentGroups.json] defines the target Deployment Groups that can be referenced in either the default Global Environment settings (in [GlobalParams.json]) or in individual stacks (in [Stacks.json]). 

If a Stack (as specified in [Stacks.json]) does not contain a specific `overrideDeploymentGroup`, then whatever 'groupCode' is set in the [GlobalParams.json] file in the 'defaultDeploymentGroup' field for that environment will be the Deployment Group it deploys to.

In [DeploymentGroups.json] you can specify any number of unique deployment groups under a unique name 'groupCode'. Within this there any number of `targetRegions` you can create to build up unique the target Org Units and/or Individual AccountIds a StackSet can will be deployed.

Within a `targetRegions` all accounts/org units will share the same AWS regions (concatinated) as determined from the associated [Accounts.json] or [OrgUnits.json] lookup. If you need very specific regions in your deployments, be careful not to combine OrgUnits or Accounts into the same `targetRegion` or it may deploy to more regions than you expect. 

This solution now supports Account Level targets for service-managed StackSets. This allows complex deployments including/excluding specific AWS Accounts using an `accountFilterType`. See this page for configuration options: [https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/account-level-targets.html]. By default this solution assumes deployments to OrgUnits and sets `AccountFilterType = "NONE" ` where `AccountIds` are not specified.

Using a combination of `accountFilterType`, `orgUnitsIds` and `accountIds` it is possible to do highly selective deployments including or excluding individual accounts in an orgUnitId.

Supported accountFilterTypes are;
- NONE (default for OrgUnit only deployments)
- INTERSECTION (default for Account deployments)
- UNION
- DIFFERENCE

#### 4.2.5 Stacks.json
[Stacks.json] is where all the magic occurs. You can list new Stacks to be made as a StackSet (type=stackset) to deploy instances to any target Org Unit or as a Stack (type=stack) to simply deploy the stack to the Deployment Account.
...
>You need to replace the outputs of CF for the below Local parameters section
	"DeploymentAccounttAWSCloudFormationStackSetAdministrationRoleArn": "arn:aws:iam::{Replace with your Deployment Account Number}:role/AWSCloudFormationStackSetAdministrationRole",
	"DeploymentAccounttLambdaVPCStackSetInstanceExecutionRoleArn": "arn:aws:iam::{Replace with your Deployment Account Number}:role/LambdaVPCStackSetInstanceExecutionRole"
...
| env[].enabled                                | [optional] 'true' or 'false' - enable or disable a stack from deploying to a particular environment                                                  |
